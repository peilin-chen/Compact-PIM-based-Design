...
Param::Param() {
	...
	clkFreq = 1e9;                      // Clock frequency 1.6974e-09
	...
	memory_bus_bandwidth = 136.512; // off-chip memory bandwidth (GB/s), 136.528GB/s->MICRON_8Gb_LPDDR5-8533_128bit_A.xml

	mode = true; // true-> take the energy and latency of off-chip memory into consideration, false: neglect off-chip memory

	pipeline_dynamic_dup = true; // true -> for full/partial pipeline processing, consider dynamic duplication method to accelerate the bottleneck layer

	// 2023.12.13 add hardware description paremeters
	numTileRow = 18; // the number of Tile rows in accelerator 
	numTileCol = 18; // the number of Tile columns in accelerator 

	numPERowPerTile = 2; // the number of PE rows in one Tile
	numPEColPerTile = 2; // the number of PE columns in one Tile

	numSubArrayRowPerPE = 2; // the number of Subarray rows in one PE
	numSubArrayColPerPE = 2; // the number of Subarray columns in one PE

	Baseline = false; // true: do not consider layer-based mapper/full-pipelined mapper/partial-pipelined mapper, just using layer-by-layer processing method (do not have layer duplication).

	Scenariotype = false; // true: only one input
	                     // false: real time inference (continuous inputs)
	...
	resistanceOff = 100e3*100;// 6e3*17;           // Roff resistance at Vr in the reported measurement dat (need to recalculate below if considering the nonlinearity)
	...
	writeVoltage = 4;					// Enable level shifer if writeVoltage > 1.5V
	//2024.01.19
